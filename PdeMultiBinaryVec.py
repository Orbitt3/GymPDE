...
class PdeMultiBinaryVec (gym.Env):
    ...
    def step(self, action):
        ...
        if self.games_counter % 250==0:
        # # interaction with low_env:
            print('len of history', len(self.history))
            # print(self.low_env.score)
            # self.dqnAgent0 = DQNAgent(self.low_env)  # self.episode_length = 100
            # self.dqnAgent1 = DQNAgent(self.low_env)
        ...
        print('self.get_reward()', self.get_reward(), 'equations_reward', equations_reward)
...
if __name__ == '__main__':
    ...
    for episode_length in [1000]:  # this is the actual intervention rate
        ...
        # buffer_length = len(model.get_buffer())
        callback = CustomCallback(env=env, verbose=1)
        model.learn(total_timesteps=15000, progress_bar=True, tb_log_name="first_run",
        ...
        # showing what did the meta agent learn
        for _ in range(1):
            env = Meta_env
            obs = env.reset()
            total_rewards = 0
